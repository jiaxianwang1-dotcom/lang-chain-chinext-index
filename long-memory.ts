...
import { MemorySaver, Annotation, StateGraph, START, END, MemorySaver as CheckpointMemory } from "@langchain/langgraph";
...
function estimateTokens(messages: BaseMessage[]): number {
  // 粗略估算：中文约 1.5 字/token，英文约 4 字符/token，取平均
  let total = 0;
...
async function compressIfNeeded(messages: BaseMessage[], summarizer: ChatOpenAI): Promise<BaseMessage[]> {
  ...
  // 保留 system prompt (第一条) + 最近 2 轮对话 (最后4条消息)
  const keepRecent = 4;
  ...
  console.log(`  [压缩完成] ${oldMessages.length} 条历史消息 → 1 条摘要，${tokenCount} → ${newTokenCount} tokens`);
...
if (scenario === "demo") {
  ...
  if (!input) {
    console.log("用法:");
    console.log("  tsx long-memory.ts demo        # 运行演示（多次对话触发压缩）");
    console.log("  tsx long-memory.ts 你好         # 自由对话");
    process.exit(0);
  }
  await chat(input);
}
